...
export default function handler(req: NextApiRequest, res: NextApiResponse<NidData | NidError>) {
  if (req.method !== 'POST')
  ...
  if (!dob.match(new RegExp(/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$/))) {
    return res.status(400).json({
    ...
    .then((data: any) => {
      if (data.success === 'true') {
        ...
        const nidData: NidData = {
          ...
          image_of_national: (trs[4].children[1] as any).children[1].attribs.src,
        };
